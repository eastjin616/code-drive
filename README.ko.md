...
**1단계** — `명령어 직접 실행` 선택 시 그룹별 다중 선택 메뉴로 이동:
...
- **그룹 헤더**(🩺 진단 / 📄 생성 / 🔍 분석) — 선택 불가, 순수 시각적 구분
- **⚡ 모두 선택** — 체크 시 권장 흐름(doctor + 생성 + review + verify + context)을 한 번에 실행
- 생성 명령어(docgen/spec/design/changelog) 완료 후 파일 열기 확인
- **doctor / verify / init / update / uninstall**은 첫 화면에서 바로 실행 가능
...
모든 생성기를 순차 실행: `docgen` → `spec` → `design` → `changelog`. 문서 재생성 후 AI 시작 문서의 CDD 컨텍스트 라우팅 블록도 최신화합니다.
